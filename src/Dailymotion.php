...
namespace baorv\dailymotion;

use baorv\dailymotion\exceptions\DailymotionApiException;
use baorv\dailymotion\exceptions\DailymotionAuthException;
use baorv\dailymotion\exceptions\DailymotionAuthRefusedException;
use baorv\dailymotion\exceptions\DailymotionAuthRequiredException;
use baorv\dailymotion\exceptions\DailymotionTransportException;
...
class Dailymotion
{
     ...
     */
    public function setGrantType($type, $apiKey, $apiSecret, array $scope = [], array $info = [])
    {
        if ($type === null) {
            $this->grantType = null;
            $this->grantInfo = [];
    ...
    protected function getCurrentUrl()
    {
        ...
        // INPUT_ENV with the filter_input() and filter_input_[] functions, so we're using a workaround there.
    ...
    public function uploadFile($filePath, $forceHostname = null, &$progressUrl = null, $callbackUrl = null)
    {
        ...
        $result = json_decode(
            $this->httpRequest($result['upload_url'], ['file' => $this->getCurlFile($filePath)]),
            true
        );
     ...
     */
    public function call($resource, $args = [])
    {
     ...
     * @param array $headers List of headers to send with the request (format `['Header-Name: header value']`).
     ...
     */
    protected function oauthRequest($url, $payload, $accessToken = null, $headers = [], &$statusCode = null, &$responseHeaders = [])
    {
        if ($accessToken !== null) {
     ...
     * @return string Response body
     */
    protected function httpRequest($url, $payload, $headers = null, &$statusCode = null, &$responseHeaders = [], $encodePayload = false)
    {
     ...
     *
     * @throws DailymotionAuthRequiredException If we can't get an access token, client need to request end-user
     *     authorization.
    ...
    protected function oauthTokenRequest(array $args)
    {
        ...
        if (empty($result)) {
            ...
            return [
                ...
                'scope' => isset($result['scope']) ? explode(chr(32), $result['scope']) : []
            ];
        } else {
     ...
     *
     * @param $filePath string path to the file to upload on the local filesystem.
     ...
     * should thus be safe as long as the API secret is kept... secret.
     *
     * @param array $session Session to store, if nothing is passed, the current session is removed from the session
     *     store.
     * @return Dailymotion `$this`
     */
    protected function storeSession(array $session = [])
    {
        if (headers_sent()) {
            if (php_sapi_name() !== 'cli') {
